use std::{fmt::Debug};
use juniper::{EmptyMutation, EmptySubscription, FieldError, FieldResult, graphql_object, graphql_value, GraphQLInputObject};
use reqwest::header::{CONTENT_TYPE, HeaderMap};
use serde::{Serialize, Deserialize};

use crate::model::{Artwork, ContestCategory, ContestListResponse, ContestSection, Currency, DisplayContest, IMatch, MatchStatus, PreRoundLockJoinedContestsResponse, RoundResponse, RoundTourResponse, Tour};
...
impl Query {
    pub async fn contest_sections(context: &Context, site: String, roundId: i32, tourId: i32) -> FieldResult<Vec<ContestSection>> {
        ...
        let roundTour:Result<RoundTourResponse, FieldError> = post(&context, &roundMessageQuery, roundTourSquad).await;
        let contestListRes:Result<ContestListResponse, FieldError> = post(&context, &contestQuery, contestList).await;
        let joinedContest:Result<PreRoundLockJoinedContestsResponse, FieldError> = post(&context, &preRoundLockQuery, contestsLite).await;
        
        match roundTour {
            Ok(res) => {
                match contestListRes {
                    Ok(contest) => {
                        match joinedContest {
                            Ok(joinContest) => {
                                let contest_section_response = contestSections(&res, &contest, &joinContest);
...
fn contestSections(roundTour: &RoundTourResponse, contestList: &ContestListResponse, joinedContest: &PreRoundLockJoinedContestsResponse) -> Vec<ContestSection>{
    let sections = &contestList.data.sections;
    let contestSectionsResponse = sections.iter().map(|section| -> ContestSection {
        ContestSection {
            ...
            displayContests: section.leagues.iter().map(|league|-> DisplayContest {
                DisplayContest {
                    _id: league.id,
                    contestType: league.contestType.to_owned(),
                    contestSize: league.contestSize,
                    currentSize: league.currentSize,
                    entryFee: Currency {amount: league.entryFee, symbol: "\\u20B9".to_owned() },
                    ...
                    prizeAmount: Currency {amount: league.prizeAmount, symbol: "\\u20B9".to_owned() },
                    prizeDisplayText: Some(convertToWordsForIndianCurrency(league.prizeAmount)),
                    showInvite: false,
                    r#match: IMatch {
